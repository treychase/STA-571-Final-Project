...
cat("Original shots_df_enhanced rows:", nrow(shots_df_enhanced), "\n")
cat("team_data rows:", nrow(team_data), "\n")
cat("Rows lost:", nrow(shots_df_enhanced) - nrow(team_data), "\n\n")
...
# If there are any NAs in streak, replace with 0 (reasonable for first shot)
team_data = team_data %>%
  mutate(streak = ifelse(is.na(streak), 0, streak))

cat("NAs in streak after replacement:", sum(is.na(team_data$streak)), "\n")
```
...
```{r hmm-model, warning=FALSE}
set.seed(571)

# Create 2-state model with streak covariate on ALL data
hmm_model = depmix(
...
# Set reasonable starting values for 2 states
hmm_model = setpars(hmm_model,
                    value = c(
                      0.5, 0.5,          # initial state probs
                      0.9, 0.1,          # transition from state 1
                      0.1, 0.9,          # transition from state 2
                      -0.2, 0.2,         # State 1: lower performance (intercept, slope)
                      0.2, 0.3           # State 2: higher performance
                    ))

# Fit with reasonable iteration limit
hmm_fit = fit(hmm_model,
              verbose = TRUE,  # Monitor progress
              emcontrol = em.control(maxit = 5000, tol = 1e-8))

# Check convergence
cat("\nConvergence message:", hmm_fit@message, "\n")
...
kable(initial_probs, col.names = "Probability",
      caption = "HMM Initial State Probabilities (2 States)", digits = 3)
...
kable(transition_matrix, caption = "HMM Transition Matrix (2x2)", digits = 3)

# Response parameters (intercepts and slopes for 2 states)
...
team_data %>%
  ...
  kable(digits = 3, caption = "Observed Statistics by HMM State (2 States)")
...
kable(streak_effects, digits = 3,
      caption = "Predicted Make Probability by State and Current Streak Length")
```
...
# Calculate predicted probabilities based on state and streak
...
kable(hmm_comparison, digits = 4,
      caption = "HMM Performance (All Teams)")
```
...
ggplot(state_sequence, aes(x = shot, y = state, color = factor(made))) +
  ...
  labs(x = "Shot Number", y = "HMM State",
       color = "Shot Made",
       title = "HMM State Transitions Over Time (2 States)") +
  theme_minimal() +
...
ggplot(team_streak_curves, aes(x = streak, color = TEAM_NAME)) +
  ...
  labs(x = "Current Streak Length",
       ...
       subtitle = "How do different teams respond to shooting streaks?") +
  theme_minimal() +
